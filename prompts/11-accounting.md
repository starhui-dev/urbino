...
读取 AGENTS.md、MASTER_PROMPT.md、progress/state.json；确认前置阶段 10 已 verified。检查代码与 git diff。只执行本阶段，不越级。
...
写 progress/reports/11.md 与 evidence/stages/11.json，更新 progress/state.json 为 implemented 或 blocked；不要自行标 verified。报告中必须给出已实现能力、真实检查结果、未验证项与风险。
