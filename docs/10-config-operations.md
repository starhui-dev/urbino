...
项目配置文件名为 `urbino.yaml`，通过 `urbino --config <path> ...` 或 `URBINO_CONFIG` 指定；应用自有环境变量使用 `URBINO_` 前缀。完整命名约定见 [项目命名规范](14-project-identity.md)。
...
应用自有 Prometheus 指标使用 `urbino_` 前缀；OpenTelemetry `service.name` 为 `urbino`，角色用受控属性区分。Go/process 等第三方标准指标不重命名。slog JSON；request_id 串联 request/attempt，字段白名单。指标至少覆盖 admission/rejection、active/waiting、upstream latency/TTFT、stream abort、quota/cooldown、refresh success/uncertain、billing pending、jobs lag、db pool、memory/goroutines。
