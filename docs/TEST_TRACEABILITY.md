...
| P01-T01 | `TestP01T01MoneyParseBoundaries`、负数/溢出、算术与直接值边界 | `internal/domain/money.go`、`internal/domain/domain_test.go`、`tests/contracts/money_contract_test.go` | `evidence/commands/phase01-contracts.txt`、`evidence/commands/phase01-focused.txt` |
| P01-T02 | request/attempt/credential 合法与终态转移；未知 attempt 可持久化但不可回退 | `internal/domain/state.go`、`internal/domain/types.go`、`internal/domain/domain_test.go`、`tests/contracts/state_transition_contract_test.go` | `evidence/commands/phase01-contracts.txt`、`evidence/commands/phase01-focused.txt` |
| P01-T03 | OpenAPI header/error/no-fake-success、validator、生成重复无 diff、闭集 capability/currency enum | `api/admin.openapi.yaml`、`api/admin_gen.go`、`api/generate.go` | `evidence/commands/phase01-openapi.txt`、`evidence/commands/phase01-generate-check.txt` |
