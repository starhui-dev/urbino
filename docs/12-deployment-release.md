...
镜像基础名称统一 `urbino`；应用服务 `urbino`，独立 Worker 服务 `urbino-worker`，命令分别为 `urbino serve` 与 `urbino worker`。镜像 owner/registry 按实际仓库确认，不自动发布。配置挂载目标使用 `urbino.yaml` 并显式传入路径，样例源文件遵守 10 文档。
