...
阶段 02 只提供存储基础，不表示计费、鉴权或网关请求链路已经完成。数据库是 PostgreSQL 18.x；应用使用 pgx，SQL 由 sqlc 生成，迁移使用 goose。迁移以增加结构为主，没有自动执行的 down。当前迁移版本为 2（`001_initial.sql` 后由 `002_stage02_guards.sql` 补充租户币种约束和价格发布边界）。
